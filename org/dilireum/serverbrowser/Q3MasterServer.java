...
public class Q3MasterServer implements MasterServer, Cloneable {
	...
	public MasterServerType		masterType;

	public Q3MasterServer(Game game, MasterServerType masterType, String url, int port, ArrayList<Integer> protocols, boolean masterEnabled) {
		masterUrl = url;
	...
	public Q3MasterServer(Game game, MasterServerType masterType, String url, int port, ArrayList<Integer> protocols, String name, boolean masterEnabled) {
		this (game, masterType, url, port, protocols, masterEnabled);
		queryGameName = name;
	}
	...
	public MasterServerType getMasterType() {
		return masterType;
	}
	...
	private class queryMasterServerThread implements Runnable {
		...
		private int buildServerList (ServerList serverList, ByteBuffer serverBuffer, int protocol) {
			...
			boolean				found;
			int					i;
			int					headerSize;			// Used to skip over the '\' characters in COD return strings

			if (masterType == MasterServerType.cod) {
				headerSize = 25;
			} else if (masterType == MasterServerType.q3m) {
				headerSize = 23;
			} else {
				headerSize = 23;
			}

			serverBuffer.get(ipbuf, 0, headerSize);
		...
		public void run () {
			...
			if (queryGameName == null) {
				...
				queryStr = queryGameName + " ";
			}

			logger.logMsg(6, "Querying master server for protocol " + protocol);
			outString = String.format("%s %s%d %s%s%s", GETSERVERS, queryStr, protocol, sq, PARAMETERS, sq);
