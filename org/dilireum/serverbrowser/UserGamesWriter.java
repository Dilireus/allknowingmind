	    	...
	    	for (Game game = games.getFirst(); game != null; game = games.getNext(game.nodeID)) {
				...
				for (int i = 0; i < game.masterList.getCount(); i++) {
					...
					if ((game.masterList.get(i).getMasterType() == MasterServerType.q3m) || 
						...
						writeElement("mastertype", qms.getMasterType().masterId);
					} else if (game.masterList.get(i).getMasterType() == MasterServerType.http) {
						HttpMasterServer qwms = (HttpMasterServer) game.masterList.get(i);
						if (!qwms.isEnabled) {
							pw.attribute("enabled", "no");
						}
						writeElement("url", qwms.masterURL);
						writeElement("mastertype", qwms.getMasterType().masterId);
					} else {
