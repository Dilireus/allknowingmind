...
public class GameDefaultsLoader extends DefaultHandler {
	...
	private String					masterTypeStr;
	private MasterServerType		masterType;
	...
	public void endElement(String uri, String localName, String qName) throws SAXException {
		...
		if (qName.equalsIgnoreCase("defaultdir")) {
			...
			protocol = new Integer(Integer.parseInt(tempVal));
			pList.add(protocol);
			...
			masterTypeStr = tempVal;
			masterType = MasterServerType.getMasterFromId(masterTypeStr);
		} else if (qName.equalsIgnoreCase("master")) {
			if ((masterType == MasterServerType.q3m) ||
				(masterType == MasterServerType.cod)) {
				if (masterNameQuery != null) {
					master = new Q3MasterServer(game, masterType, masterAddress, masterPort, pList, masterNameQuery, masterEnabled);
				} else {
					master = new Q3MasterServer(game, masterType, masterAddress, masterPort, pList, masterEnabled);
				}
			} else if (masterType == MasterServerType.http) {
				master = new HttpMasterServer(game, masterType, masterAddress, masterEnabled);
			}
