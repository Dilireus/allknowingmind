...
public class GameSetupDialog extends Dialog {
		...
		NewMaster.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if ((ms = msd.open()) != null) {
					...
					masterUrl.removeAll();
					for (int i = 0; i < selectedGame.masterList.getCount(); i++) {
						if ((selectedGame.masterList.get(i).getMasterType() == MasterServerType.q3m) || 
							 (selectedGame.masterList.get(i).getMasterType() == MasterServerType.cod)) {
							Q3MasterServer qms = (Q3MasterServer) selectedGame.masterList.get(i);
							masterUrl.add(qms.masterUrl);
						} else if (selectedGame.masterList.get(i).getMasterType() == MasterServerType.http) {
							HttpMasterServer ems = (HttpMasterServer) selectedGame.masterList.get(i);
	...
	private void displayItem(Game game) {
		...
		for (int i = 0; i < game.masterList.getCount(); i++) {
			if ((game.masterList.get(i).getMasterType() == MasterServerType.q3m) ||
				(game.masterList.get(i).getMasterType() == MasterServerType.cod)) {
				Q3MasterServer qms = (Q3MasterServer) game.masterList.get(i);
				masterUrl.add(qms.masterUrl);
			} else if (game.masterList.get(i).getMasterType() == MasterServerType.http) {
				HttpMasterServer ems = (HttpMasterServer) game.masterList.get(i);
				masterUrl.add(ems.masterURL);
			}
		}
		...
		for (int i = 0; i < selectedGame.masterList.getCount(); i++) {
			if ((selectedGame.masterList.get(i).getMasterType() == MasterServerType.q3m) ||
				...
				if (qms.masterUrl.equalsIgnoreCase(url)) {
					...
					masterType.setText(qms.getMasterType().masterName);
				...
				if (ems.masterURL.equalsIgnoreCase(url)) {
					...
					masterType.setText(ems.getMasterType().masterName);
