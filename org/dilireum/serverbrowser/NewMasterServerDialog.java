	...
	private void createContents(final Shell shell) {
		...
		masterType.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				} else if (mType.equals(MasterServerType.q3m.masterName) ||
							mType.equals(MasterServerType.cod.masterName) ||
							mType.equals(MasterServerType.q2m.masterName) ||
							mType.equals(MasterServerType.doom3.masterName)) {
		...
		okButton.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				if (masterType.getText().equals(MasterServerType.http.masterName)) {
					newMaster = new HttpMasterServer(game, MasterServerType.http, masterUrl.getText(), enabled.getSelection());
				} else if (masterType.getText().equals(MasterServerType.q3m.masterName)) {
					ArrayList<Integer>	pList = makeProtocolArray(protocols.getText());
					if (queryName.getText().length() == 0) {
						newMaster = new Q3MasterServer(game, MasterServerType.q3m, masterUrl.getText(), Integer.parseInt(port.getText()), 
								pList, enabled.getSelection());
					} else {
						newMaster = new Q3MasterServer(game, MasterServerType.q3m, masterUrl.getText(), Integer.parseInt(port.getText()), 
								pList, queryName.getText(), enabled.getSelection());
					}
				} else if (masterType.getText().equals(MasterServerType.cod.masterName)) {
					ArrayList<Integer>	pList = makeProtocolArray(protocols.getText());
					newMaster = new Q3MasterServer(game, MasterServerType.cod, masterUrl.getText(), Integer.parseInt(port.getText()), 
							pList, enabled.getSelection());
				} else if (masterType.getText().equals(MasterServerType.cod.masterName)) {
					ArrayList<Integer>	pList = makeProtocolArray(protocols.getText());
					newMaster = new Q3MasterServer(game, MasterServerType.cod, masterUrl.getText(), Integer.parseInt(port.getText()), 
