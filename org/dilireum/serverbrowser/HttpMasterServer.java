...
public class HttpMasterServer implements MasterServer, Cloneable {
	...
	public HttpMasterServer(Game game, MasterServerType masterType, String url, boolean masterEnabled) {
		masterURL = url;
		setGame(game);
    ...
    public MasterServer clone() {
        try {
            return (HttpMasterServer) super.clone();
	...
	public void queryMasterServer(ServerList serverList) {
		...
		try {
			...
			for (line = buff.readLine(), count = 1; line != null; line = buff.readLine(), count++) {
